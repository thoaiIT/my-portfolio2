    ...
    const updateAnimations = () => {
      ...
      elements.forEach((el, index) => {
        ...
        if (rect.bottom < 0 || rect.top > windowHeight) {
          ...
          // Fading in
          targetProgress =
            1 - (rect.top - fadeInEnd) / (fadeInStart - fadeInEnd);
          ...
          // Fading out
          targetProgress =
            (rect.bottom - fadeOutEnd) / (fadeOutStart - fadeOutEnd);
        } else {
        ...
        progressRefs.current[index] +=
          (targetProgress - progressRefs.current[index]) * smoothingFactor;

        // Apply smooth easing
        const easedProgress = gsap.parseEase('power2.inOut')(
          progressRefs.current[index]
        );
        ...
        // Manage will-change
        if (
          progressRefs.current[index] > 0.01 &&
          progressRefs.current[index] < 0.99
        ) {
